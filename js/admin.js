...
function renderSetupRequired() {
  adminView.innerHTML = `<section class="admin-login"><div class="admin-login-copy"><a class="back-link" href="../">← Volver a la plataforma</a><span class="admin-kicker">CENTRO DE CONTROL</span><h1>Falta conectar Firebase.</h1><p>La interfaz administrativa ya está instalada. Solo debes completar la configuración del proyecto y desplegar las funciones.</p></div><div class="admin-login-card"><span class="admin-lock">⚙</span><h2>Configuración pendiente</h2><p>Edita <code>public/js/firebase-config.js</code>, activa Authentication y despliega Firestore + Functions siguiendo el README.</p><a class="button button-outline" href="../">Ver plataforma pública</a></div></section>`;
}

function renderAccess(status = {}) {
  const body = status.canClaim
    ? `<span class="admin-lock">✓</span><h2>Activar administrador único</h2><p>Firebase Authentication contiene solamente esta cuenta. Al continuar quedará registrada como el único administrador de Certifica+.</p><button class="button button-primary" type="button" id="claimAdminButton">Activar administración</button><button class="button button-outline" type="button" data-admin-action="signout">Cerrar sesión</button><p class="admin-form-message" id="adminClaimMessage" aria-live="polite"></p>`
    : `<span class="admin-lock">!</span><h2>Acceso no autorizado</h2><p>La cuenta <strong>${escapeHTML(status.email || '')}</strong> no coincide con el administrador único. ${Number(status.authUserCount || 0) > 1 ? 'Hay más de un usuario registrado en Authentication; elimina los adicionales desde Firebase Console.' : ''}</p><button class="button button-outline" type="button" data-admin-action="signout">Cerrar sesión</button>`;
  adminView.innerHTML = `<section class="admin-login"><div class="admin-login-copy"><a class="back-link" href="../">← Volver a la plataforma</a><span class="admin-kicker">CENTRO DE CONTROL</span><h1>Configura cada capacitación.</h1><p>Este espacio está separado del portal público. Solo la única cuenta registrada en Firebase Authentication puede administrar la plataforma.</p></div><div class="admin-login-card">${body}</div></section>`;
  select('#claimAdminButton')?.addEventListener('click', claimAccess);
}

function renderLogin() {
  adminView.innerHTML = `<section class="admin-login"><div class="admin-login-copy"><a class="back-link" href="../">← Volver a la plataforma</a><span class="admin-kicker">CENTRO DE CONTROL</span><h1>Administración privada.</h1><p>Inicia sesión con la cuenta creada en Firebase Authentication. Conocer esta URL no concede acceso.</p></div><div class="admin-login-card"><span class="admin-lock">⌾</span><h2>Iniciar sesión</h2><form id="adminLoginForm" class="admin-auth-form"><label>Correo<input type="email" name="email" autocomplete="username" required></label><label>Contraseña<input type="password" name="password" autocomplete="current-password" minlength="6" required></label><button class="button button-primary" type="submit">Entrar</button><p class="admin-form-message" id="adminLoginMessage" aria-live="polite"></p></form></div></section>`;
...
function renderUnavailable() {
  adminView.innerHTML = `<section class="admin-login"><div class="admin-login-copy"><a class="back-link" href="../">← Volver a la plataforma</a><span class="admin-kicker">CENTRO DE CONTROL</span><h1>No pudimos abrir el panel.</h1></div><div class="admin-login-card"><span class="admin-lock">!</span><h2>Servicio no disponible</h2><p>Revisa tu conexión e inténtalo nuevamente.</p><button class="button button-primary" type="button" data-admin-action="retry">Reintentar</button></div></section>`;
}
...
async function claimAccess() {
  ...
  try {
    ...
    select('#adminClaimMessage').textContent = 'No fue posible activar la administración. Authentication debe contener exactamente un usuario: esta cuenta.';
...
function renderDashboard() {
  ...
  adminView.innerHTML = `<section class="admin-shell">
    <aside class="admin-sidebar">
      <div><span class="admin-kicker">CERTIFICA+ ADMIN</span><h2>Capacitaciones</h2><p>Edita el contenido completo o crea una nueva capacitación.</p></div>
      ...
      <div class="admin-side-actions"><a href="../">Ver plataforma pública</a><button type="button" data-admin-action="signout">Cerrar sesión</button></div>
    </aside>
    <div class="admin-workspace">
      <header class="admin-header"><div><span class="admin-kicker">${isNew ? 'NUEVA CAPACITACIÓN' : 'EDITAR CAPACITACIÓN'}</span><h1>${isNew ? 'Crear un curso' : escapeHTML(course.title)}</h1></div>${isNew ? '<button class="button button-outline" type="button" data-admin-action="cancel-new">Cancelar</button>' : `<a class="button button-outline" href="../#curso/${encodeURIComponent(course.courseId)}" target="_blank" rel="noopener noreferrer">Vista del estudiante ↗</a>`}</header>
      ...
      <form class="admin-editor admin-course-editor" id="courseConfigForm" novalidate>
        ...
        <section>
          ...
          <div class="resource-upload-panel">
            <div class="resource-cover-preview">${cover ? `<img src="${escapeAttr(api.resourceUrl(cover.id))}" alt="Portada de ${escapeAttr(course.title)}">` : `<span>${escapeHTML(course.icon || '✦')}</span><small>Sin imagen de portada</small>`}</div>
        ...
        <div class="admin-save-bar"><p id="adminSaveMessage" aria-live="polite">${isNew ? 'Completa todos los campos para publicar el nuevo curso.' : 'Los cambios se aplicarán en todos los dispositivos.'}</p><button class="button button-primary" type="submit">${isNew ? 'Crear y publicar curso' : 'Guardar todos los cambios'}</button></div>
      </form>
